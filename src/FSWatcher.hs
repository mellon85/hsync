...
iterateDirectory x c = do
    statement <- liftIO $ DB.sql_SELECT_MODTIME c
        ...
        send path isDir = do
            ...
            case modTime of
                ...
                Right t -> do
                    ...
                    case ret of
                        True -> return ()
                        _    -> yield $! Left (Info path isDir t False Nothing)
...
md5 = awaitForever $ hashit
    where
        hashit x | isDirectory x = yield x
                 | otherwise     = do
                    h <- liftIO . liftM hashlazy . BL.readFile $ entryPath x
